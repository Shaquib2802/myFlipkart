...
const Details = () => {
  ...
  return (
    <div className=" flex flex-col justify-center items-center">
      <img src={data?.images} className="w-48 h-48 " />
      <div>{slug}</div>
      {/* <div onClick={()=>Navigate(`/try`)}>Add </div> */}
      {data?.reviews?.map((i, ind) => (
        <div
          key={ind}
          onClick={() => Navigate("/try", { state: i })}
          className="border shadow-xl p-4 w-[35%] "
        >
          <div className="flex ">
            <img
              src={data.images}
            ...
            <div className="flex flex-col mt-2 ml-3 leading-5">
              <div className="font-semibold">{i?.reviewerName}</div>
              <div>{i?.reviewerEmail}</div>
              <div className="w-[100%] text-xs">{i?.date}</div>
              <div className="mt-1" onClick={()=>Navigate(`/try/${i?.rating}`)}>
                <Rating name="read-only" value={i?.rating} readOnly />
              </div>
            </div>
          </div>
          <div className="w-[100%] ml-5 ">{i?.comment}</div>
        </div>
      ))}
    </div>
  );
